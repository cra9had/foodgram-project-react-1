...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not request.data.get("image"): # по документации изображение 
# обязательно но в случае обновления рецепта может не потребоваться
            test_base64code = (
    ...
    def favorite(self, request, pk=None):
        if request.method == "POST":
            model = Favorite
            error_data = {"errors": "Рецепт уже был добавлен в избранное"}
            return self._do_post_method(request, model, error_data)
        model = Favorite
        error_data = {"errors": "Рецепт уже был удален из избранного"}
    ...
    def shopping_cart(self, request, pk=None):
        if request.method == "POST":
            model = ShoppingCart
            error_data = {"errors": "Рецепт уже был добавлен в корзину"}
            return self._do_post_method(request, model, error_data)
        model = ShoppingCart
        error_data = {"errors": "Рецепт уже был удален из корзины"}
        return self._do_delete_method(request, model, error_data)
